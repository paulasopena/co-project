...
By adding the encrypted version of the circuit ID (`circID`) just before this newly padded packet, we have the full version of the decrypted data.
...
Once a message is received, the system will verify what is the command of the cell. There are two main cases:
...
response confirming that the TCP connection was established. On the other end, unlike the actual full-fledged TOR, where there can be as many ORs as wanted, we implement a single OR waiting. This OR, when receiving a response from a request it simply forwarded, it will just encrypt the packet and send it back to the original sender.

### Onion Proxy Implementation
#### Folder Structure and Roles for the Proxy
The proxy implementation is organized into two main files, `op.py` and `op_utils.py`, each handling different aspects of the onion routing system:
...
<div align="center">
    <img src="https://github.com/user-attachments/assets/cc5ed022-f6aa-4105-9026-bd0b37dc6e69" alt="Control and Relay Cells" width="800"/>
  </div>
...
This file will simply run a Docker file and create a container which will run our program. The Python library `cryptography` may need to be manually installed.
...
1. Dingledine, R. (n.d.). *Tor: The Second-Generation Onion Router*. Retrieved from [https://svn-archive.torproject.org/svn/projects/design-paper/tor-design.pdf](https://svn-archive.torproject.org/svn/projects/design-paper/tor-design.pdf)
2. How does Tor really work? The definitive visual guide. (2023). (n.d.). Retrieved October 15, 2024, from [https://skerritt.blog/how-does-tor-really-work/](https://skerritt.blog/how-does-tor-really-work/)
